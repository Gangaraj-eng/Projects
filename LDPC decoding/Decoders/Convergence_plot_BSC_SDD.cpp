...
int number_of_errors(Node *vns, int *Transmitted, int n)
{
  int nerr = 0;
  for (int i = 0; i < n; i++)
  {
...
int main(int argc, char const *argv[])
{
    ...
    for (int pe = 0; pe < 11; pe++)
    {
        ...
        {
            int *Recieved = Bsc_channel(Transmitted, Number_of_VNs, pv[pe]);
            for (int i = 0; i < Number_of_VNs; i++)
            {
                if (Recieved[i] == 1)
                    pIntial[i] = 1 - pv[pe];
            ...
            {
                errors[iteration_index]+=number_of_errors(Variable_Nodes,Transmitted,Number_of_VNs);
                iteration_index++;
                for (int i = 0; i < Number_of_CNs; i++)
                {
                    cn = Check_Nodes + i;
    ...
    FILE *gnupipe = NULL;
    char *GnuCommands[] = {"set title \"Convergence plot for BSC Soft Decision\"",
                           "set key outside",
                           "set yrange [0:1.2]",
                           "set xrange [0:30]",
